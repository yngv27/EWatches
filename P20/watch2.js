...
function init() {
    ...
    const startX=[-5,50,115,170],startY=[60,60,60,60];
    optFilled = true;
    const hht = 40;
    const vht = 40;
  
    function Poly(arr, fill) {
      if (!fill) g.fillPoly(arr, false);
    ...
    ************/
    ...
    let drawTimer = () => {
        // erase area
      g.clearRect(20,30,200,48);
        let diff = Math.floor(Date().getTime()/1000) - timer; 
        let mm = Math.floor(diff / 60).toString().padStart(2,'0');
        let ss = (diff % 60).toString().padStart(2, '0');
        LF.drawString(mm+" "+ss, 90, 30);
        if(diff % 300 == 0) Bangle.buzz();
    };

    let startTimer = () => {
        timer = Math.floor(Date().getTime()/1000);
        let tik = wOS.on("tick", drawTimer);
    };
  
    let stopTimer = () => {
        wOS.removeListener("tick", drawTimer);
        timer = 0;
    };
    ...
    let drawData = (d) => {
      ...
      g.setColor("#a0a0a0");
      for(let x=0; x<5; x++) {
        g.clearRect( 86+x*16,6,96+x*16, 9);
        if(batt < x*20) 
