...
//sleepMode:()=>{TC.writeByte(0xA5,0x03);},
  ...
  CSTx16.prototype.touchevent = function()  {
    var p = this.getXY();
    //print(p);
    if(p.gest) this.handleGesture(p);
    else {
      if (p.b && !this.down) {
        this.fx = p.x; this.fy = p.y; this.down = true;
        try { wOS.time_left = wOS.ON_TIME; //reset LCD on time.
            } catch(err) {}
      }
      if (!p.b && this.down) {
  ...
  CSTx16.prototype.handleGesture = function(p) {
      try { wOS.time_left = wOS.ON_TIME; //reset LCD on time.
          } catch(err) {}
      if (p.gest==C.CLICK) this.emit("touch",p);
      else if (p.gest>=1 && p.gest<=4) this.emit("swipe",p.gest); 
      else if (p.gest==C.LONG) this.emit("longtouch",p); 
  };
  ...
  */
  ...
  //var TC = exports.connect({i2c: I2C1, rst: D39, intr: D32}); //QYGold
  //I2C1.setup({sda:D6, scl:D7, bitrate:200000});
  //var TC = exports.connect({i2c: I2C1, rst: D10, intr: D28}); //P4S
